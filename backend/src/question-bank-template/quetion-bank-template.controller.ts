...
export class QuestionBankTemplateController {
    ...
    constructor(private readonly templateService: QuestionBankTemplateService){}

    @Get()
    @HttpCode(HttpStatus.OK)
    @ApiOkResponse({ type: QuestionBankTemplateDto, isArray: true })
    @UseGuards(AuthGuard)
    getTemplates() {
    ...
    @Get(':id')
    @HttpCode(HttpStatus.OK)
    @ApiOkResponse({ type: QuestionBankTemplateDto })
    @UseGuards(AuthGuard)
