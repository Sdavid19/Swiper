...
export class VoteService {
  ...
  async getAllVotesUserParticipatedIn(userId: number): Promise<VoteDto[]> {
...
async getTopStats(voteId: number) {
  ...
  const totalUserCount = new Set(answers.map(a => a.userId)).size;
  ...
  .map(([questionId, questionStat]) => ({questionId, ...questionStat,}))
  .sort((a, b) => b.yes - a.yes || a.question.text.localeCompare( a.question.text));

  return { stats, userCount: totalUserCount } as AnswerTopStatsDto;
}
