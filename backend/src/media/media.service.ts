...
export class MediaService implements OnModuleInit {
    ...
    async fetchMovies() {
        ...
        for (const p of platforms) {
            const showsByCurrentPlatform = await client.showsApi.searchShowsByFilters({
                showType: 'movie',
                catalogs: [p.id],
                country: countryCode,
                orderBy: 'popularity_1week',
            });
            mediaToSave.push(...(showsByCurrentPlatform.shows));
        }
        return mediaToSave;
    }
    ...
    async fetchAndSavePlatforms() {
        ...
        await this.prisma.platform.createMany({
            data: services.map(p => ({name: p.id, imageUrl: p.imageSet.lightThemeImage})),
            skipDuplicates: true
        });
    }
